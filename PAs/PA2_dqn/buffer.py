...
class ReplayBuffer:
    def __init__(self, capacity, state_size, seed, device):
        ...
        # Create tensors without pin_memory to avoid CUDA errors
    ...
    def add(self, transition):
        ...
        self.states[self.idx] = torch.as_tensor(state, dtype=torch.float32)
        self.actions[self.idx] = torch.as_tensor(action, dtype=torch.long)
        ...
        self.dones[self.idx] = torch.as_tensor(done, dtype=torch.int)
    ...
    def sample(self, batch_size):
        # using np.random.default_rng().choice is faster
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def update_priorities(self, data_idxs, priorities):
        # Convert priorities to numpy array if it's a list
        if isinstance(priorities, list):
            priorities = np.array(priorities, dtype=np.float32)
        # Add eps and apply alpha
