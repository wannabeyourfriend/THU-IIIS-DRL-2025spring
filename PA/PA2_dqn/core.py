...
import logging
import numpy as np
from buffer import ReplayBuffer, PrioritizedReplayBuffer, PrioritizedNStepReplayBuffer
...
def train(cfg, env, agent, buffer, seed, log_dict):
    ...
    for step in range(1, cfg.timesteps + 1):
        ...
        if step > cfg.batch_size + cfg.nstep:
            if isinstance(buffer, PrioritizedReplayBuffer) or isinstance(buffer, PrioritizedNStepReplayBuffer):
                batch, weights, tree_idxs = buffer.sample(cfg.batch_size)
                loss, td_error, Q = agent.update(batch, step, weights=weights)

                # Convert td_error to a list instead of using numpy
                td_error_list = td_error.cpu().tolist()
                buffer.update_priorities(tree_idxs, td_error_list)
            elif isinstance(buffer, ReplayBuffer):
                batch = buffer.sample(cfg.batch_size)
                loss, td_error, Q = agent.update(batch, step)
            else:
                raise RuntimeError(f"Unknown buffer type: {type(buffer)}")
