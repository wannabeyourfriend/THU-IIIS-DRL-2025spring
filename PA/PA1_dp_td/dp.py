...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, eps=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            new_value_function[s] = sum(p_trans * (reward + gamma * value_function[next_state] * (1 - terminal)) for p_trans, next_state, reward, terminal in P[s][policy[s]])
        if np.max(np.abs(new_value_function - value_function)) < eps:
            break
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    # YOUR IMPLEMENTATION HERE #
    for s in range(nS):
        Qsa_list = np.zeros(nA)
        for a in range(nA):
            Qsa_list[a] = sum(p_trans * (reward + gamma * value_from_policy[next_state] * (1 - terminal)) for p_trans, next_state, reward, terminal in P[s][a])
        new_policy[s] = np.argmax(Qsa_list)
    ############################
    return new_policy
...
def value_iteration(P, nS, nA, gamma=0.9, eps=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            for a in range(nA):
                Qsa[s, a] = sum(p_trans * (reward + gamma * value_function[next_state] * (1 - terminal)) for p_trans, next_state, reward, terminal in P[s][a])
        new_value_function = Qsa.max(axis=1)
        if np.max(np.abs(new_value_function - value_function)) < eps:
            break
        value_function = new_value_function
    best_actions = [np.where(Qsa[s] == Qsa[s].max())[0] for s in range(nS)]
    policy = np.array([np.random.choice(actions) for actions in best_actions])
    ############################
    return value_function, policy
